...
- Construct buildings, train units, scout, adapt, fight, and win
...
- Instead of selecting a race as in other strategy games, each player selects a set of features (see the feature section)
- A map composed of multiple hexagon tiles is generated
- Each hexagon has no or a limited number of harvestable resources and a terrain type
- Each player starts with a headquarters building and 3 workers on their starting hexagon
- Each player receives starting resources

## Turn Structure

- Command Phase: All players simultaneously issue their commands
...
- Initialization
  - Each player starts with a headquarters building, 3 workers on a single hexagon
...
- Constructing a building takes one or multiple turns
...
- The terrain type of and buildings on hexagons can influence the movement range of passing units

#### Upgrade Units

- Buildings not training units can research upgrades
- Building researching upgrades cannot train units at the same time
- Researching upgrades takes one or multiple turns
...
- Cancel (handle before other steps to make the resources of cancelled orders available for other commands)
  - Cancelled constructions, trainings and upgrades are removed and resources reimbursed
- Construction
  - Construction progress on all hexagons
  - If the required number of turns is reached, the construction completes
  - Cancelled constructions are removed and resources reimbursed
- Training
...
- Resources are gathered from all eligible hexagons
...
- Combat has three phases
  - 1 Counter Phase
    ...
    - Remove the number of countered units from the opponent's army
  - 2 Strength Phase
...
- There are four levels of visibility
  - Unseen: All hexagons are unseen during the start of the game
...
- Feature selection
  ...
  - Draft System: Players alternate selecting features, possibly including veto for banning features
...
- Async games:
  - Each player has a fixed amount of time per turn
...
- Web API
  - Might require an updated client
  - Might require an updated database
- Client
